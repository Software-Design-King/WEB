...
const TeacherGradesPage: React.FC = () => {
  ...
  const handleEditScore = (
    ...
    if (scoreData?.scoresByGradeAndSemester) {
      const semesterData =
        scoreData.scoresByGradeAndSemester[grade]?.[semester];
      if (semesterData?.subjects && Array.isArray(semesterData.subjects)) {
        semesterData.subjects.forEach((subjectData: any) => {
  ...
  const handleSaveAllScores = async () => {
    try {
      ...
      if (subjectsToUpdate.length > 0) {
        // API 호출
        const response = await editStudentScore(
          selectedStudent,
          semesterNumber,
          subjectsToUpdate
        );
  ...
  const handleRegisterScore = async () => {
    ...
    if (
      newSubjects.some((subject) => subject.score < 0 || subject.score > 100)
    ) {
    ...
    try {
      ...
      if (response.code === 20000) {
        ...
        if (updatedScoreData.code === 20000) {
          ...
          // 가공된 성적 데이터도 다시 처리
          const { processedData, allSubjects, allExamTypes } =
            transformScoreData(updatedScoreData.data);
          setProcessedScoreData(processedData);
          setSubjectList(allSubjects);
          setExamTypeList(allExamTypes);
        }
              ...
              <>
                <ControlsContainer>
                  <Select
                    ...
                    value={selectedStudent ? selectedStudent : undefined}
                    ...
                    <Row style={{ width: "100%" }}>
                      ...
                      <Col span={12}>
                        <InputNumber
                          ...
                          value={subject.score}
                          onChange={(value) =>
                            handleSubjectScoreChange(index, value as number)
                          }
                          style={{ width: "100%" }}
                        />
                      </Col>
              ...
              {newSubjects.map((subject, index) => (
                ...
                >
                  <Select
                    ...
                    onChange={(value) =>
                      handleSubjectChange(index, "examType", value)
                    }
                    style={{ width: "120px" }}
                  >
